...
! end check compatibility ice shelf/bathy
   ...
   SUBROUTINE zgr_sco
      ...
      INTEGER  ::   iip2, ijp2, iim2, ijm2   ! temporary integers
      ...
      DO WHILE( jl <= 10000 .AND. ( zrmax - rn_rmax ) > 1.e-8_wp ) !  Iterative loop  !
         ...
         DO jj = 1, nlcj
            DO ji = 1, nlci
               ...
               iip2 = MIN( ji+2, nlci )      ! force zri = 0 on last line (ji=ncli+1 to jpi)
               ijp2 = MIN( jj+2, nlcj )      ! force zrj = 0 on last raw  (jj=nclj+1 to jpj)
               iim1 = MAX( ji-1, 1 )
               ijm1 = MAX( jj-1, 1 )
!CEOD This is the original line (0)
               IF( (zenv(ji,jj) > 0._wp) .AND. (zenv(iip1,jj) > 0._wp)) THEN
!CEOD               IF( (zenv(ji,jj) > 50._wp) .AND. (zenv(iip1,jj) > 50._wp) ) THEN
                  zri(ji,jj) = ( zenv(iip1,jj  ) - zenv(ji,jj) ) / ( zenv(iip1,jj  ) + zenv(ji,jj) )
               END IF
!CEOD This is the original line
               IF( (zenv(ji,jj) > 0._wp) .AND. (zenv(ji,ijp1) > 0._wp)) THEN
!CEOD               IF( (zenv(ji,jj) > 50._wp) .AND. (zenv(ji,ijp1) > 50._wp) ) THEN
         ...
         DO jj = 1, nlcj
            DO ji = 1, nlci
               ...
               iim1 = MAX( ji-1, 1 )
               ijm1 = MAX( jj-1, 1 )
!CEOD NEED THIS FOR ZENV AVOIDENCE IN WAD               IF( (zenv(ji,jj) > 50._wp) .AND. (zenv(iip1,jj) > 50._wp) .AND.  (zenv(iim1,jj) > 50._wp) .AND. (zenv(ji,ijp1) > 50._wp).AND. (zenv(ji,ijm1) > 50._wp)) THEN
!CEOD            IF( (zenv(ji,jj) > 20._wp) .AND. (zenv(iip1,jj) > 20._wp) .AND.  (zenv(iim1,jj) > 20._wp) .AND. (zenv(ji,ijp1) > 20._wp).AND. (zenv(ji,ijm1) > 20._wp)) THEN
...
!================================================================================
      ...
      END DO
      !
      ! CEOD modify stretch to take away ref depth : 
! move this so we only do it for mesh mask not for configuration then we dont need
! to run twice
...
!! assume we force it to be 1 meter then its easy
