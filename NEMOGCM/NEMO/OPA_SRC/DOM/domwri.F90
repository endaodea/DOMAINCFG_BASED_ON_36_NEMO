MODULE domwri
   ...
   !!   dom_uniq       :
   !!   dom_stiff_tool      : diagnose maximum grid stiffness/hydrostatic consistency (s-coordinate)
   !!----------------------------------------------------------------------
   ...
   PUBLIC   dom_stiff_tool           ! routine called by inidom.F90
...
CONTAINS
   ...
   SUBROUTINE dom_wri
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) :: zdepu, zdepv,zdepf
      !!----------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('dom_wri')
      !
      CALL wrk_alloc( jpi, jpj, zprt, zprw )
      CALL wrk_alloc( jpi, jpj, jpk, zdepu, zdepv,zdepf )
      !
      IF(lwp) WRITE(numout,*)
...
!CEOD Modify gdept/w for mesh mask output so that we can make weights 
!      DO jj = 1, jpj
!         DO ji = 1, jpi
!           if(scobot(ji,jj)-rn_wd_ref_depth > 1 ) then
!              DO jk = 1, jpkm1
!                   gdept_0(ji,jj,jk) = gdept_0(ji,jj,jk) *(  ( scobot(ji,jj)-rn_wd_ref_depth )/( scobot(ji,jj) ) )
!                   gdepw_0(ji,jj,jk) = gdepw_0(ji,jj,jk) *(  ( scobot(ji,jj)-rn_wd_ref_depth )/( scobot(ji,jj) ) ) 
!              END DO
!           else
!              DO jk = 1, jpkm1
...
!            !gdepw_0 (ji,jj,jk) = (scosrf(ji,jj)+hbatt(ji,jj))*z_gsigw3(ji,jj,jk) ! assume we force it to be 1 meter then its easy
!                   gdept_0(ji,jj,jk) = (REAL(jk-1,wp)+0.5)/REAL(jpk-1,wp) 
!                   gdepw_0(ji,jj,jk) =  REAL(jk-1,wp)     /REAL(jpk-1,wp) 
!              ENDDO
!           endif
!         END DO
!      END DO

         CALL lbc_lnk( gdept_0, 'T', 1. )   
         CALL iom_rstput( 0, 0, inum4, 'gdept_0', gdept_0, ktype = jp_r4 )     
!CEOD Need U and V 
            DO jk = 1,jpk
               DO jj = 1, jpjm1
                  DO ji = 1, fs_jpim1   ! vector opt.
                     zdepu(ji,jj,jk) = 0.5*(gdept_0(ji,jj,jk) + gdept_0(ji+1,jj ,jk) )
                     zdepv(ji,jj,jk) = 0.5*(gdept_0(ji,jj,jk) + gdept_0(ji ,jj+1,jk) )

                     zdepf(ji,jj,jk) = 0.25*(gdept_0(ji,jj,jk) + gdept_0(ji+1 ,jj+1,jk) +gdept_0(ji+1,jj  ,jk)  + gdept_0(ji  ,jj+1,jk) ) 
                  END DO
               END DO
            END DO
            ...
            CALL iom_rstput( 0, 0, inum4, 'gdepf', zdepf, ktype = jp_r4 )
!CEOD
      ...
      CALL wrk_dealloc( jpi, jpj, jpk, zdepu, zdepv,zdepf )
      !
...
!!             so that the ratio is computed at the same point (i.e. uw and vw) ....
      ...
      IF(lwp) THEN
         WRITE(numout,*)
         WRITE(numout,*) 'dom_stiff_tool : maximum grid stiffness ratio: ', zrxmax
         WRITE(numout,*) '~~~~~~~~~'
      ENDIF
      !
   END SUBROUTINE dom_stiff_tool
   !!======================================================================
END MODULE domwri
